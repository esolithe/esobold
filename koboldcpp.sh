...
if [[ $1 == "rebuild" ]]; then
	...
	bin/micromamba run -r conda -p conda/envs/linux pyinstaller --noconfirm --onedir --collect-all customtkinter --collect-all jinja2 --collect-all psutil --collect-all pdfplumber --collect-all PyMuPdf --collect-all tqdm --collect-all chardet --collect-all openai --collect-all tiktoken --collect-all prompt_toolkit --collect-all msgpack --collect-all numpy --collect-all pyyaml --collect-all json-repair --collect-all aiofiles --collect-all python-ulid --collect-all requests --collect-all flask --collect-all discord.py --collect-all python-telegram-bot --collect-all matrix-nio --add-data "./esoExtras/opticlaw:./esoExtras/opticlaw" --add-data './koboldcpp.py:.' --add-data './json_to_gbnf.py:.' --version-file './version.txt' --clean --console koboldcpp.py -n "koboldcpp-launcher"
	if [ "$KCPP_CUDA" = "rocm" ]; then
		if [ ! -n "$ROCM_PATH" ]; then
			ROCM_PATH=/opt/rocm
		fi
